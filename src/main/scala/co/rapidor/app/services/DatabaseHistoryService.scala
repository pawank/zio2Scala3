...
object DatabaseHistoryService {
  def getDatabaseHistoryList: RIO[DatabaseHistoryService, List[FlywaySchemaHistory]] = ZIO.serviceWithZIO(_.getDatabaseHistoryList)
  def getDatabaseHistory(version: String): RIO[DatabaseHistoryService, Option[FlywaySchemaHistory]] = ZIO.serviceWithZIO(_.getDatabaseHistory(version))
}
...
object DatabaseHistoryServiceLive {
  val layer: URLayer[Console, DatabaseHistoryService] = (DatabaseHistoryServiceLive(_)).toLayer[DatabaseHistoryService]
}
