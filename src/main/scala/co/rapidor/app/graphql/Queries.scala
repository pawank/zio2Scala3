...
object QueriesApi {
  case class Queries(
      databaseHistoryList: RIO[DatabaseHistoryService, List[FlywaySchemaHistory]],
  ...
  val api: GraphQL[Console with Clock with DatabaseHistoryService] =
    graphQL(
      RootResolver(
        Queries(
          DatabaseHistoryService.getDatabaseHistoryList,
          args => DatabaseHistoryService.getDatabaseHistory(args.version),
        )
      )
    ) @@
